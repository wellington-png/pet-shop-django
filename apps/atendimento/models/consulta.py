...
class Consulta(BaseModel):
    ...
    veterinario = ForeignKey(
        "account.Veterinario", verbose_name="Veterinário", on_delete=CASCADE
    )
    ...
    valor_total = DecimalField(
        max_digits=18, decimal_places=2, verbose_name="Valor Total"
    )
    ...
    def valor_consulta(self):
        return (
            self.itemconsulta_set.all().aggregate(valor_total=Sum("valor_item"))[
                "valor_total"
            ]
            + self.valor_total
        )
